...
void calculate_yield_gradient(const double theta,
                               ...
                               const double dtheta_dsig[VOIGTSIZE_3D], double grad[VOIGTSIZE_3D]) // Output gradient vector
{
    ...
    const double dC1_dtheta = alpha * beta * cos(3 * theta) * sin(acos(beta * sin(3.0 * theta)) / 3.0 - gamma * PI / 6.0) / sqrt(1.0 - beta*beta * pow(sin(3.0 * theta),2.0));
