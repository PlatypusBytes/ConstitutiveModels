...
def test_strain_controlled_compression_oedometer():
    ...
    orig_stress_vector = np.array([-1, 0, 0])
    ...
    model_loc = os.path.join(project_dir,'build_C','lib','linear_elastic_with_vertical_tens_cutoff_3d_interface.'+extension)
    ...
    incr_driver = IncrDriver(orig_stress_vector,
                             ...
                             const_model_info,
                             3,
                             5, ndim=3, n_direct_stress_components=1, n_shear_components=2,
...
def test_stress_controlled_extension_below_cutoff():
    ...
    # Define the original stress vector
    orig_stress_vector = np.array([0, 0, 0])
    ...
    strain_increment = np.array([0, 0, 0])
    stress_increment = np.array([333,667, 667])
    ...
    incr_driver = IncrDriver(orig_stress_vector,
                             ...
                             const_model_info,
                             3,
                             5, ndim=2, n_direct_stress_components=1, n_shear_components=2,
                             vertical_axis_index=0)

    incr_driver.solve([1,1, 1])

    expected_strains = np.array([-stress_increment / np.array([M,G, G]),
                                 -2 * stress_increment / np.array([M,G, G]),
                                 -3* stress_increment / np.array([M,G, G])])
...
def test_strain_controlled_tension_oedometer():
    ...
    # Define the original stress vector
    orig_stress_vector = np.array([-1.0, 0.0, 0.0])
    ...
    # strain increment per time step in the form of [eps_x, eps_y, eps_z, gamma_xy, gamma_xz, gamma_yz]
    vertical_strain_increment = 1e-4
    strain_increment = np.array([vertical_strain_increment, 0.0, 0.0])
    stress_increment = np.zeros_like(strain_increment)
    ...
    expected_strains = np.array([[vertical_strain_increment, 0.0,0.0],
                                    [2*vertical_strain_increment, 0.0,0.0],
                                    [3*vertical_strain_increment, 0.0,0.0]])
