...
!  Main program that\_calls\_umat ( performs calculation writing  to output.txt).
     ...
     &  /),(/6,6/))
      ...
      real(8), parameter,dimension(1:6,1:6)::MRoscImT=MRoscI            !  latest $\cM^{-T}$ (is orthogonal)
       ...
       real(8), parameter,dimension(1:6,1:6)::MRendul=reshape          !  M for isomorphic Rendulic $ sigma_{11}= -T_{11}$,  $sigma_{22}  = -(T_{22} + T_{33}) / \sqrt(2) $,  $ Z= \dots$
...
![4.2]  loop over keywords(1) unless keyword(1) = *Repetition  it is copied to keyword(2) which is the true type of loading
     ...
     &          9H kiter = ,i2, 8H TEMP = ,f9.4, 8H TIME = ,f9.4)')
     &          ikeyword , kstep, kinc, kiter ,TEMP,TIME(1)                    ! AN 2023 temperat
...
!    used by  utility routine SPRINC  or SPRIND for iterative diagonalization
      ...
      subroutine ReadStepCommons(from, ninc, maxiter,deltaTime,
     .                           deltaTemp, every)   ! AN 2023 temperat
