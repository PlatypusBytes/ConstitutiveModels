...
class YieldFunctions:
    ...
    def F_mohr_coulomb(sigma, phi, c):
        ...
        f = (2 * sigma[0,0] - sigma[1,1] - sigma[2,2])/4  - (2* sigma[0,0] + sigma[1,1] + sigma[2,2])/4 * np.sin(phi) - c * np.cos(phi)
        return f

    @staticmethod
    def dfds_mohr_coulomb(sigma, phi):
        """Derivative of the Mohr-Coulomb yield function with respect to the stress tensor."""
        dfdsigma = np.zeros((3, 3))
        dfdsigma[0, 0] = 0.5 - 0.5 * np.sin(phi)
        dfdsigma[1, 1] = -0.25 - 0.25 * np.sin(phi)
        dfdsigma[2, 2] = -0.25 - 0.25 * np.sin(phi)
        return dfdsigma
    ...
    def HS_cap_F_c(sigma,p, p_p, phi, M):
        ...
        q_special = sigma[0, 0] + (alpha - 1) * sigma[1, 1] - alpha * sigma[2, 2]
    ...
    def cap_df_dsigma( sigma, p, q, M):
        ...
        if q < 1e-12:
            ...
            return (dp / 3.0) * np.eye(3)
        s = sigma - p * np.eye(3)
        dq = 2.0 * q / (M**2)
        # ∂q/∂σ = (3/2) s/q
        dq_dsigma = (3.0/2.0) * s / q
        dfdsigma = (dp / 3.0) * np.eye(3) + dq * dq_dsigma
        return dfdsigma
    ...
    def dgd_dsigma(sigma, p, q):
        ...
        Pdev = np.einsum('ik,jl->ijkl', I, I) \
               - (1 / 3) * np.einsum('ij,kl->ijkl', I, I)

        term1 = (3 / (2 * q)) * Pdev
        term2 = (9 / (4 * q ** 3)) * np.einsum('ij,kl->ijkl', s, s)
